...
(function (cbImagePaste, $, undefined) {
	...
	cbImagePaste.showPasteDialog = function () {
		...
		dlg_buttons[cbImagePaste.cbp_txt_btn_cancel] = function () {
			$(this).dialog("close");
		};
	...
	function createPanel() {
		//imageEditor.deactivateTool("crop")
		...
		var panel = new fabric.Canvas(canvasHtml);
		panel.setWidth(boxw)
		panel.setHeight(boxh)
		...
		// create a rectangle object
					...
					panel.add(rect);
		panel.setBackgroundImage(pastedImage.src)


		$("#cbp_panel_box").append(canvasHtml);
		//imageEditor.activateTool("crop")
		//showPreview();
	};
	...
	$(document).ready(function () {
		...
		imageEditor = {
			...
			deactivateTool: function (name) {
				if (this.activeTool != name) return
				this.tools[name].deactivate.apply(this)
				this.activeTool = false
			},
			createToolButton: function (name) {
				return $("<a>", {
					html: name,
					href: "#",
					click: function () {
			...
			addToolbar: function () {
				var entriesColumnOne = ["crop", "arrow", "freehand", "text"]
				var entriesColumnTwo = ["marker", "pencil"]
				var columnOne = $("<div>")
				var columnTwo = $("<div>")
				var row = $("<div>")
				columnOne.append(entriesColumnOne.map(this.createToolButton))
				columnTwo.append(entriesColumnTwo.map(this.createToolButton))
		...
		imageEditor.tools = {
			"crop": {
				activate: function () {
					return;
					var panel = $("#cbp_panel_box canvas");
			...
			"arrow": {
				activate: function () {
					var panel = $("#cbp_panel_box canvas");
					var canvas = new fabric.Canvas(panel[0]);
					canvas.setBackgroundImage(pastedImage.src);
					// create a rectangle object
					var rect = new fabric.Rect({
					  left: 100,
					  top: 100,
